...
@Component({
  ...
  template: `
    <div class="flex-1 w-full max-w-6xl mx-auto md:shadow-xl flex flex-col md:flex-row relative z-0 md:surface md:rounded-2xl md:overflow-clip h-full min-h-0">
      
      <!-- Left Panel: Branding & Calendar -->
      <div class="w-full md:w-1/3 glass-brand p-6 text-white flex flex-col gap-4 md:gap-8 relative shrink-0">
      ...
      <!-- Right Panel: Form -->
      <div class="w-full md:w-2/3 bg-white/40 dark:bg-zinc-900/40 relative h-full flex flex-col md:overflow-hidden min-h-0">
        <form [formGroup]="form" (ngSubmit)="submit()" class="flex flex-col flex-1 w-full min-h-0">
          
          <div class="flex-1 md:overflow-y-auto hidden-scrollbar p-6 pb-2 flex flex-col gap-4 md:gap-6">
          ...
          <div class="shrink-0 p-6 flex gap-4 mt-auto sticky bottom-0 z-20 bg-white/40 dark:bg-zinc-900/40 md:bg-transparent backdrop-blur-md md:backdrop-blur-none">
            <button uiButton variant="ghost" type="button" class="flex-1 bg-white/50 dark:bg-zinc-800/50 md:bg-transparent" routerLink="/customer">CANCEL</button>
            <button uiButton variant="primary" type="submit" class="flex-1" [disabled]="form.invalid">SUBMIT</button>
          </div>
        </form>
      </div>
